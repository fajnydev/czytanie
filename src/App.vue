<template>
  <div class="app">
    ...
    <SuperModal v-if="showModal"
                :blacklist="blacklist"
                :progress-letters="progressLetters"
                :progress-syllables="progressSyllables"
...
export default {
  ...
  computed: {
    progressLetters() {
      return this.makeProgress(this.stats.letters);
    },
    progressSyllables() {
      return this.makeProgress(this.stats.syllables);
    }
  },
  methods: {
    /* wspólna funkcja budująca listę {text, correct, wrong, avgMs} */
    makeProgress(src) {
      if (!src) return [];
      return Object.entries(src).map(([txt, d]) => {
        const tries = d.correct + d.wrong;
        return {
          text: txt,
          correct: d.correct,
          wrong: d.wrong,
          avgMs: Math.round(d.total / tries)
        };
    ...
    addToBlacklist(w) {
      if (!w) return;
      if (!this.blacklist.includes(w)) {
        this.blacklist.push(w);
      }
      localStorage.setItem('readingBlacklist', JSON.stringify([...this.blacklist]));
    },
    restoreWord(w) {
      this.blacklist = this.blacklist.filter(wo => wo !== w);
      localStorage.setItem('readingBlacklist', JSON.stringify([...this.blacklist]));
    },
    resetQueues() {
      this.syllableQueue = [...this.syllablesBase];
      this.wordQueue = this.wordsBase.map(w => ({...w}));
    },
    pickRandom(a) {
    ...
    pickWeightedLetter() {
      ...
      const bucket = this.lettersBase.map(l => {
        ...
        const w = Math.max(1, 5 - c);
        total += w;
        return [l, w];
      });
      let r = Math.random() * total;
      for (const [ltr, w] of bucket) {
        if (r < w) return ltr;
        r -= w;
      }
    ...
    nextItem() {
      ...
      do {
        if (this.immediateQueue.length) {
          ...
          item = {text: this.pickWeightedLetter(), type: 'letter'};
        } else if (mode === 'syllables') {
          item = {text: this.pickRandom(this.syllableQueue), type: 'syllable'};
        } else {
          item = {...this.pickRandom(this.wordQueue), type: 'word'};
        }
